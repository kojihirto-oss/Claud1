...
#### VAULT
- **定義**: 機密情報や生成物を安全に保管する場所。
- **使用例**: 「APIキーはVAULTに保存する」
- **詳細**: Part07（セキュリティ設計）

#### RELEASE
- **定義**: 検証済みのSSOTスナップショット。不変の成果物として提供される。
- **使用例**: 「四半期ごとにRELEASEを作成する」
- **詳細**: Part13（リリース管理）

#### WORK
- **定義**: SSOTを更新するための一連の作業単位。タスクやチケットとして管理される。
- **使用例**: 「このWORKはPart02の修正を目的とする」
- **詳細**: Part04（作業管理）

#### RFC (Request for Comments)
- **定義**: 仕様や設計に関する意見を求めるためのドキュメント。ADRの前段階。
- **使用例**: 「新しいPermission TierについてRFCで意見を募集する」
- **詳細**: Part14（変更管理）

#### VIBEKANBAN
- **定義**: プロジェクトのタスク（WORK）を可視化・管理するためのカンバンボード。
- **使用例**: 「VIBEKANBANでタスクの進捗を確認する」
- **詳細**: Part04（作業管理）

#### Context Pack
- **定義**: AIエージェントに渡すためのコンテキスト情報（プロンプト、ファイル、過去の対話履歴など）をまとめたもの。
- **使用例**: 「Part09を修正するためのContext Packを作成する」
- **詳細**: Part08（AIエージェント連携）

#### Patchset
- **定義**: 複数の変更（コミット）をまとめたもの。レビューや適用を容易にするために使用される。
- **使用例**: 「Part02の修正内容をPatchsetとして提出する」
- **詳細**: Part05（ソースコード管理）
