...
## 追加セクション: False Positive例外管理とSBOM/Attestation運用

### False Positiveの例外管理（期限・証跡）

#### 例外管理フロー
| ステップ | アクション | 責任者 | 期限 | 証跡 |
|---------|---------|--------|------|------|
| **1. 申請** | ADRで除外申請を作成 | 開発者 | 検出時 | ADRファイル |
| **2. 審査** | セキュリティ担当が審査 | セキュリティ担当 | 48時間 | ADRコメント |
| **3. 承認** | HumanGateで承認 | チームリーダ | 72時間 | ADR承認記録 |
| **4. 設定** | 除外ルールを適用 | 開発者 | 承認後 | コミットログ |
| **5. 定期見直し** | 6ヶ月ごとに再評価 | セキュリティ担当 | 6ヶ月 | ADR更新 |

#### 除外期限の種類
| 除外種別 | 最大期限 | 更新条件 | 証跡要件 |
|---------|---------|---------|---------|
| **一時的除外** | 6ヶ月 | 更新不可 | ADRに理由と期限 |
| **テスト用** | 無期限 | 明示的マーク必須 | テストコード内 |
| **False Positive** | 1ヶ月 | 要再審査 | ADRに判断根拠 |
| **サンプルデータ** | 無期限 | example/test接頭辞 | コード内マーク |

#### 期限切れ除外の対応
| 状態 | アクション | 責任者 | 期限 |
|------|---------|--------|------|
| **期限切れ** | 除外ルールを削除 | 開発者 | 即時 |
| **継続必要** | 再申請して承認 | 開発者 | 期限前1週間 |
| **未対応** | CIブロックを再有効化 | CIシステム | 自動 |

---

### SBOM/Attestationの最小運用（証跡の残し方）
...
| **実行者** | リリース担当、セキュリティ担当 |
| **実行内容** | TrivyでSBOM生成 |
| **保存先** | evidence/sbom/YYYYMMDD_HHMMSS_sbom.json |
| **ログ内容** | 生成日時、対象イメージ、コンポーネント一覧 |
| **保存期間** | 3年間 |
...
| **実行者** | リリース担当 |
| **実行内容** | Sigstoreで署名 |
| **保存先** | evidence/attestation/YYYYMMDD_HHMMSS_attestation.json |
| **ログ内容** | 署名日時、署名者、対象成果物、署名fingerprint |
| **保存期間** | 3年間 |

#### 証跡の最小構成（必須項目）
- SBOM: 生成日時、ツール、成果物、コンポーネント一覧、脆弱性
- Attestation: 署名日時、署名者、対象成果物、署名fingerprint

#### 証跡の確認方法
| 確認項目 | 方法 |
|---------|------|
| **SBOM整合性** | SHA256ハッシュ照合 |
| **署名検証** | Sigstore検証 |
| **保存確認** | ファイル存在確認 |
| **期間確認** | 3年間保存されているか |
