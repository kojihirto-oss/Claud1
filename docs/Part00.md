...
本リポジトリは **「設計書SSOT（Design Document Single Source of Truth）」** として維持・運用される。
本 Part00 は **SSOT運用の憲法** として、以下を保証する：
...
### R-0010: フォルダ構成と役割・コミット基準【MUST】
本リポジトリの構成要素は以下の役割と扱いを厳守する：

| フォルダ | 役割 | 扱い・コミット基準 |
| :--- | :--- | :--- |
| **docs/** | **設計書SSOT**（最重要） | **Commit必須**。ここが正本。 |
| **checks/** | **検証スクリプト** | **Commit必須**。品質ゲートを守る。 |
| **evidence/** | **証跡・監査ログ** | **Commit必須**。Verify結果やdiffを残す。巨大ログは除外。 |
| **sources/** | **原文・一次情報** | **Commit推奨**（改変禁止）。`research_inbox` 等の巨大ファイルは除外。 |
| **decisions/** | **意思決定記録(ADR)** | **Commit必須**。変更の根拠。 |
| **_imports/** | **外部依存・ローカル** | **Commit禁止**（.gitignoreを確認）。リポジトリに含めない。 |
| **out/** | **一時生成物** | **Commit禁止**（.gitignoreを確認）。いつでも削除可能にする。 |

**コミット禁止物**:
- 巨大な生ログ（MB単位）→ 要約または抜粋のみ evidence に残す
- 1GBを超えるバイナリ
- 機密情報（API Key等）
- 再生成可能な一時ファイル（`out/`）

**根拠**: [FACTS_LEDGER F-0004](FACTS_LEDGER.md)（フォルダ構造）

---

### R-0011: 変更ループとエージェント運用【MUST】
変更は必ず以下の **最短ループ** を遵守する：

1. **Research**: 現状確認・情報収集
2. **Evidence**: 変更前の状態を記録
3. **Edit**: 最小範囲での変更
4. **Verify**: 検証実行（`checks/verify_repo.ps1`）
5. **Commit**: 証跡と共にコミット
6. **PR/Merge/Release**: 統合

**エージェント作業ルール**:
- **予期せぬ変更の検知**: 作業中に意図しないファイル変更や破壊を検知したら、即座に作業を停止する。
- **指示待ち**: 迷ったら勝手に進めず、人間に判断を仰ぐ（HumanGate）。

**根拠**: [Part15](Part15.md)（運用ループ）、[Part06](Part06.md)（安全運用）
...
- [x] フォルダ役割・コミット基準（R-0010）が明記されているか
- [x] 変更ループ・エージェント運用（R-0011）が明記されているか
